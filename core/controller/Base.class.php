...
abstract class Base
{
    ...
    public function __construct()
    {
        ...
        $this->view->assign(array(
			...
			'diy_url' => $this->site_config['diy_url'],
...
//			if (in_array($t['formtype'], array('checkbox', 'files', 'diy'))) $data[$t['field']] = array2string($data[$t['field']]);
    ...
    protected function listSeo($cat, $page = 1)
    {
        ...
        $seo_title = $page > 1 ? $cat['catname'] . '第' . $page . '页' . ','.$this->site_config['site_title'] : $seo_title . $this->site_config['site_title'];
    ...
    protected function showSeo($data, $page = 1)
    {
        ...
        $seo_title = $data['title'] . ',' . ($page > 1 ? '第' . $page . '页' . ' - ' : '') . $this->site_config['site_title'];
		$this->site_config['site_title'];
        $seo_keywords =  $this->site_config['site_keywords'] ;
	 
         $seo_description = empty($data['description']) ? $this->site_config['site_description']  : $data['description'];
		$this->site_config['site_description'];
        return array('site_title' => $seo_title, 'site_keywords' => $seo_keywords, 'site_description' => $seo_description);
    }
