...
class field
{
    ...
    public static function editor($name, $content = '', $setting = '')
    {
		...
		KindEditor.ready(function(K) {
                window.editor = K.create('#" . $name . "', { 
				" . $other . "
				items:[" . $source . "'forecolor','bold','italic','underline','lineheight','table','|','fontname','fontsize','code','plainpaste','wordpaste','|','image','multiimage','flash','media','insertfile','link','unlink','|','justifyleft','justifycenter','justifyright','justifyfull','insertorderedlist','insertunorderedlist','indent','outdent','clearhtml','quickformat','hr','baidumap'" . $page . ",'|','fullscreen']
			});
        });
		</script>";
        } else {
		...
		KindEditor.ready(function(K) {
                window.editor = K.create('#" . $name . "', { 
				" . $other . "
				items:[" . $source . "'forecolor','bold','italic','underline','table','|','image','multiimage','flash','media','insertfile','link','unlink','fontname','fontsize','|','justifyleft','justifycenter','justifyright','justifyfull','clearhtml','quickformat','baidumap','|','fullscreen']
			});
        });
		</script>";
        }
    ...
    public static function file($name, $content = '', $setting = '')
    {
        ...
        setTimeout(function(){
          ...
          Button.uploadify({
            ...
            uploader      : '/yiwangxiao/index.php?c=uploadfile&a=uploadify_upload&type=" . $setting['type'] . "',
            method   : 'post',
            formData : {'submit' : '1','session_id' : '" . session_id() . "'},
            fileTypeExts: '" . $type . "',
            fileObjName:'file',
            buttonText: '" . $txt . "',
            queueSizeLimit: 1,
