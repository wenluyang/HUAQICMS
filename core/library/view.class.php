...
class view
{
	...
	protected function getalltags($num)
	{	
		$db = xiaocms::load_class('Model');
		$xiaotags=$db->query('select keywords from `#xiaocms_content`')->fetchAll();						
		foreach($xiaotags as $t)
		{
			foreach($t as $key)
			{
				$alltags=$alltags.$key.",";
		...
		//echo $alltags;		
		$alltags=substr($alltags,0,strlen($alltags)-1);
		...
		if($nums>$num)
			...
			$num=$nums;			
		for ($i=0;$i<$num;$i++){
				echo '<a href="'.url('index/tags',array('tag'=>$alltags[$i])).'" title="'.$alltags[$i]. '">'.$alltags[$i].'</a> ';	
		}
	}
