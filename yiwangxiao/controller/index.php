...
class index extends Admin {
	...
	public function treeAction() {
		...
		$cats =  $this->db->setTableName('category')->getAll(null, null, array('catid', 'catname','parentid','typeid','http'),'listorder DESC,catid ASC');
		$categorys = array();
		if(!empty($cats)) {
			foreach($cats as $r) {
				if($r['typeid']==1) {
	...
	public function mainAction() {
		...
		$client_url = '/index.php?c=index&a=license&'.$data;
