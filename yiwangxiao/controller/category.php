...
class category extends Admin {
	    ...
	    if ($this->post('submit')) {
	        ...
	        if ($this->post('addall')) {//批量添加
				...
				foreach ($names as $val) {
				    list($catname, $catdir,$catenname) = explode('|', $val);
					...
					$data['catdir']  =str_replace(" ","",trim($catdir));
					$data['catenname'] = trim($catenname);
				...
				$data['catdir']  =str_replace(" ","",$data['catdir']);
	...
	public function cacheAction() {
	    ...
	    foreach ($data as $t) {
	        ...
	        $category[$catid]['content'] = keyword_link(htmlspecialchars_decode($category[$catid]['content']));
	    }
