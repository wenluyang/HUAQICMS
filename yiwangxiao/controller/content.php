...
class content extends Admin {
	...
	public function addAction() {
	    ...
	    if ($this->post('submit')) {
			...
			if ($data['hits']==0)$data['hits']=mt_rand(200,500);
	        ...
	        $msg = '添加成功' . '<div style="padding-top:10px;">' . $msg . '</div>';
			//百度推送代码			
			$sitedata  =  $this->site_config;			
			$api = $sitedata['site_baidusub'];
			$api = str_replace('amp;','',$api);
			if (strlen($api)>35)
			{
					$urls ='http://'.$_SERVER['HTTP_HOST'].$this->view->get_show_url($data);
					$ch = curl_init();
					$options =  array(
						CURLOPT_URL => $api,
						CURLOPT_POST => true,
					...
					{
						$baidumsg=json_decode($result);
						$temp='';
						foreach($baidumsg as $key=>$t)
						{
							if($key=='remain')
								$temp.= '今日剩余：'.$t.'条  ';
							if($key=='success')
							...
							$msg = $msg.'<br/>百度推送成功！<br/>链接：'.$urls.'<br/>'.$temp;		
					}
					else
					{
							$msg = $msg.'<br/>百度推送失败！';	
					...
					$this->show_message($msg, 1,url('content/add', array('catid'=>$data['catid'])),3000);
			}
			else
			{
				$this->show_message($msg, 1,url('content/add', array('catid'=>$data['catid'])),3000);
	...
	private function additionalAction($data) {
		...
		if(empty($data['keywords'])){
			$data['keywords']=getPostData("http://blog.51cto.com/user/keywords.php",array("title"=>$data['title'],"content"=>strip_tags($content)));
			$data['keywords'] = str_replace(array('，',' '), ',', $data['keywords']);		
		}
