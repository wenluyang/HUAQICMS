	...
	<div class="pad-10">
		<div class="col-tab">
			<ul class="tabBut cu-li">
				...
				<li onClick="tab(7,2);" id="tab_2" class="<?php if ($type==2) { ?>on<?php } ?>">系统设置</li>
				<?php if (defined('XIAOCMS_MEMBER')) {?>
				<li onClick="tab(7,3);" id="tab_3" class="<?php if ($type==3) { ?>on<?php } ?>">会员配置</li>
				<?php } ?>
                <li onClick="tab(7,5);" id="tab_5" class="<?php if ($type==5) { ?>on<?php } ?>">邮件配置</li>
				<li onClick="tab(7,4);" id="tab_4" class="<?php if ($type==4) { ?>on<?php } ?>">URL配置</li>
				</ul>
			...
			<table width="100%" class="table_form">
				...
				<tr>
					...
					<label  onclick='$("#close").hide();'>&nbsp;&nbsp;&nbsp;<input name="data[site_mobile]" type="radio" value="0" <?php if (empty($data['site_mobile'])) { ?>checked<?php } ?> /> 关闭</label>					
					</td>
				</tr>
				...
				<tr>
					<th width="100">百度推送接口： </th>
					<td><input class="input-text" type="text" name="data[site_baidusub]" value="<?php echo $data['site_baidusub']; ?>" size="70"/><div class="onShow">百度主动推送提交接口，获取地址：http://zhanzhang.baidu.com/linksubmit/index</div></td>
				</tr>
			</table>
			</div>
