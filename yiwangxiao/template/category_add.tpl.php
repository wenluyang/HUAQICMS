		...
		<input type="hidden" value="<?php echo $catid; ?>" name="catid">
				<div class="col-tab">
					...
					<tr>
						...
						<div class="onShow">格式：栏目名称|栏目目录|栏目英文名称 一行一个 如： 新闻|xinwen|NEWS</div>
						</td>
					</tr>
					</tbody>
					<?php } ?>
					...
					<tr style="display: none">
						<th width="105"> 英文名称：</th>
						<td><input type="text" class="input-text" size="30" value="<?php echo $data['catenname']; ?>" name="data[catenname]" id="endir"></td>
					</tr>
					<tr>
					...
					<tr >
						<th>单页面内容：</th>
						<td>
						<?php  $field = new field();  echo $field->editor('content', $data['content'], array('system'=>1)); ?>
						</td>
					</tr>
					...
					<tr>
						<th>栏目图片：</th>
						<td><?php echo $field->file('image',$data['image'],array('type'=>'gif,jpg,jpeg,png','preview'=>1,'size'=>2,'buttontxt'=>'上传图片')); ?></div>
						</td>
					</tr>
					<tr>
						<th >seo标题：</th>
...
function ajaxdir() {
	...
	if (dir == '') {
		...
		$.post('../index.php?c=api&a=trans', { word:$('#dir').val() }, function(data){ $('#dir_text').val(data.replace(/\s+/g,"_")); });
	}
	var endir = $('#endir').val();
	if (endir == '') {
	    $.post('../index.php?c=api&a=trans', { word:$('#dir').val() }, function(data){ $('#endir').val(data); });
	}
}
